...
class TestColabClient(unittest.TestCase):
    ...
    def test_assign_creates_new(self, mock_get_assignment, mock_post_assignment):
        mock_get_assignment.return_value = MagicMock(
            endpoint=None,
        ...
        self.assertIsInstance(result, AssignmentHandle)
        self.assertTrue(result.is_new)
        self.assertEqual(result.endpoint, DEFAULT_ASSIGNMENT.endpoint)
        self.assertEqual(result.runtime_proxy_info.token, "mock-token")

    @patch("colab_mcp.client.ColabClient._get_assignment")
    def test_assign_reuses_existing(self, mock_get_assignment):
        mock_get_assignment.return_value = MagicMock(
        ...
        result = self.client.assign(NOTEBOOK_HASH, Variant.GPU)

        self.assertIsInstance(result, AssignmentHandle)
        self.assertFalse(result.is_new)
        self.assertEqual(result.endpoint, "existing-endpoint")
