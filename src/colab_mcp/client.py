...
class AssignmentHandle(BaseModel):
    ...
    machine_shape: Shape = Shape.STANDARD
    variant: AssignmentVariant = AssignmentVariant.DEFAULT
    is_new: bool = False
...
class GetAssignmentResponse(BaseModel):
    ...
    url: str | None = None
    runtime_proxy_info: RuntimeProxyInfo | None = Field(
        default=None, alias="runtimeProxyInfo"
    )
    variant: Variant | AssignmentVariant | None = Field(default=None, alias="variant")
    acc: str | None = Field(default=None, alias="acc")
    nbh: str | None = Field(default=None, alias="nbh")
...
class ColabClient:
    ...
    def assign(
        ...
        existing_assignment = self._normalize_existing_assignment(assignment)
        if existing_assignment is not None:
            return existing_assignment
        if not assignment.token:
            raise ColabRequestError(
                "Assignment lookup did not return a reusable runtime handle or an XSRF token.",
                request=None,
                response=None,
            )
        ...
        return AssignmentHandle(
            ...
            runtimeProxyInfo=res.runtime_proxy_info,
            accelerator=res.accelerator,
            machine_shape=res.machine_shape,
            variant=res.variant,
            is_new=True,
        )
    ...
    def _normalize_existing_assignment(
        self, assignment: GetAssignmentResponse
    ) -> AssignmentHandle | None:
        runtime_proxy_info = assignment.runtime_proxy_info
        if runtime_proxy_info is None and assignment.endpoint and assignment.token:
            proxy_url = assignment.url or assignment.endpoint
        ...
        if not assignment.endpoint or runtime_proxy_info is None:
            return None

        variant = assignment.variant
        if isinstance(variant, Variant):
            variant = {
                Variant.DEFAULT: AssignmentVariant.DEFAULT,
                Variant.GPU: AssignmentVariant.GPU,
                Variant.TPU: AssignmentVariant.TPU,
            }[variant]
        if variant is None:
            variant = AssignmentVariant.DEFAULT

        accelerator = Accelerator.NONE
        if assignment.acc and assignment.acc in Accelerator._value2member_map_:
            accelerator = Accelerator(assignment.acc)
