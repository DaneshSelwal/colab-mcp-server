...
async def check_session_proxy_tool_fn(ctx: Context = CurrentContext()) -> bool:
    ...
    del ctx
    return bool(fe_connected)


check_session_proxy_tool = Tool.from_function(
    fn=check_session_proxy_tool_fn,
    name=INJECTED_TOOL_NAME,
    description="Checks whether a Google Colab proxy session is already connected. Returns a boolean representing whether the headless proxy connection is available.",
)
...
class ColabSessionProxy:
    ...
    async def start_proxy_server(self):
        ...
        self.middleware.append(ColabProxyMiddleware(self.proxy_client))
    ...
    def get_connected_client(self):
        if not self.is_connected():
            return None
        return self.proxy_client.proxy_mcp_client

    def build_connect_url(self, notebook_url: str | None = None) -> str | None:
        if self.wss is None:
            return None
        base_url = notebook_url or f"{COLAB}{SCRATCH_PATH}"
        parsed = urlparse(base_url)
        fragment_values = dict(parse_qsl(parsed.fragment, keep_blank_values=True))
        fragment_values["mcpProxyToken"] = self.wss.token
        fragment_values["mcpProxyPort"] = str(self.wss.port)
        return urlunparse(parsed._replace(fragment=urlencode(fragment_values)))
